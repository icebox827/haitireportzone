...
<header class="header header2 headerstyleone headerstyleoneseven">
	<nav class="navbar navbar-default<?php if(($container_bg_white==1) || ($global_white_conainter==1)){ if(($pionusnews_body_bg_img) || ($pionusnews_body_bg_img_to)){ ?> navstyle-for-bg<?php } } ?>">
		<div class="container">
			...
			?>
			<a title="<?php esc_html_e('Profile', 'pantograph'); ?>" class="pantograph-login" href="<?php echo ''.home_url().'/edit-profile'; ?>"><div class="login pull-right"></div></a>
			<?php } else { ?>
			<div class="login pull-right login-pantograph"></div>
			<?php } } ?>
