...
?>
<ul id="scroller_layout_3" class="<?php echo esc_attr( $ulclass ).' '.esc_attr( $layout ).' '.esc_attr( $show_on ); ?>"><?php
foreach( $media_array as $item ) {
		echo '<li><a href="' . esc_url( $item['link'] ) . '" target="' . esc_attr( $target ) . '"><img src="' . esc_url( $item[$size] ) . '"  alt="Instagram Photo"/></a></li>';
}
?></ul>
<style>
	<?php include('css/layout_3_css.php'); ?>
</style>
<?php
if($show_on=='quarter'){
if ($link != '') { ?>
...
(function($) {
	$(function() {
		$("#scroller_layout_3").simplyScroll();
	});
})(jQuery);
</script>
<?php } ?>