...
class pantograph_menu_logo {
	...
	public function __construct() {
		// Register init
		add_action( 'init', array( $this, 'pantograph_menu_logo_init' ) );
	 ...
	 * @uses pantograph_menu_logo_admin() To call admin functions.
	 * @uses add_action() To hook function.
	 ...
	 */
	public function pantograph_menu_logo_init() {
		...
		if ( is_admin() ) {
			...
			new pantograph_menu_logo_admin();
		}
		...
		if ( apply_filters( 'nmi_filter_menu_item_content', true ) ) {
			add_filter( 'nav_menu_css_class',       array( $this, 'pantograph_menu_logo_register_menu_item_filter'   ), 15, 3 );
			add_filter( 'walker_nav_menu_start_el', array( $this, 'pantograph_menu_logo_depantograph_menu_logo_register_menu_item_filter' ), 15, 2 );
		}
	 ...
	 */
	public function pantograph_menu_logo_load_textdomain() {
		/* If translation isn't loaded, load it */
		if ( ! is_textdomain_loaded( 'pantograph_menu_logo' ) )
			load_plugin_textdomain( 'pantograph_menu_logo', false, dirname( $this->plugin_basename ) . '/languages' );
	}
	 ...
	 */
	public function pantograph_menu_logo_register_menu_item_filter( $item_classes, $item, $args = array() ) {
	 ...
	 */
	public function pantograph_menu_logo_depantograph_menu_logo_register_menu_item_filter( $item_output, $item ) {
 ...
 * @since 1.0
 *
 * @uses pantograph_menu_logo To initialize plugin.
 */
function pantograph_menu_logo_instantiate() {
	new pantograph_menu_logo();
}
add_action( 'plugins_loaded', 'pantograph_menu_logo_instantiate', 15 );
