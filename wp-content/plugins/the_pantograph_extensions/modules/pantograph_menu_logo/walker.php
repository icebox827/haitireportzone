	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( has_post_thumbnail( $item_id ) ) {
			...
			if ( version_compare( $wp_version, '3.5', '>=' ) ) {
				...
				$remove_link = ' | <a href="#" data-id="' . $item_id . '" class="nmi_remove" onclick="NMIRemoveThumbnail(\'' . $ajax_nonce . '\',' . $item_id . ');return false;">' . esc_html__( 'Remove menu item image', 'pantograph_menu_logo' ) . '</a>';
			}
		} else {
			$output .= '<div class="nmi-current-image nmi-div" style="display: none;"></div>';
			$link_text = __( 'Upload menu item image', 'pantograph_menu_logo' );
		}
