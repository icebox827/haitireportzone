...
if(!class_exists('Redux_Options') ){
   ...
   $fslashed_dir = trailingslashit(str_replace('\\','/', dirname(__FILE__)));
    ...
    class Redux_Options {
        ...
        protected $framework_version = '3.6.0.4';
        ...
        */
        function __construct($sections = array(), $args = array(), $extra_tabs = array()) {    
            ...
            $defaults['menu_title'] = esc_html__('Options', 'pantograph');
            $defaults['page_icon'] = 'icon-themes';
            $defaults['page_title'] = esc_html__('Options', 'pantograph');
            ...
            $defaults['footer_credit'] = sprintf(wp_kses( __('<span id="footer-thankyou">Options panel created using <a href="%s" target="_blank">Redux Framework</a> v%s</span>', 'pantograph'), $allowed_html_array ), $this->framework_url, $this->framework_version);
        ...
        */
        function _enqueue() {
            ...
            wp_localize_script('redux-opts-js', 'redux_opts', array('reset_confirm' => esc_html__('Are you sure? You will lose all custom values.', 'pantograph'), 'opt_name' => $this->args['opt_name']));
        ...
        */
        function _options_page_html() {
            ...
            echo ' &nbsp; ';
            submit_button(esc_html__('Reset to Defaults', 'pantograph'), 'secondary', $this->args['opt_name'] . '[defaults]', false);
            ...
            if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('redux-opts-saved') == '1') {
                if(isset($this->options['imported']) && $this->options['imported'] == 1) {
                    echo '<div id="redux-opts-imported">' . apply_filters('redux-opts-imported-text-' . esc_attr( $this->args['opt_name'] ), wp_kses( __('<strong>Settings Imported!</strong>', 'pantograph'), $allowed_html_array )) . '</div>';
                } else {
                    echo '<div id="redux-opts-save">' . apply_filters('redux-opts-saved-text-' . esc_attr( $this->args['opt_name'] ), wp_kses( __('<strong>Settings Saved!</strong>', 'pantograph'), $allowed_html_array )).'</div>';
                }
                delete_transient('redux-opts-saved');
            }

            echo '<div id="redux-opts-save-warn">' . apply_filters('redux-opts-changed-text-' . esc_attr( $this->args['opt_name'] ), wp_kses( __('<strong>Settings have changed, you should save them!</strong>', 'pantograph'), $allowed_html_array )) . '</div>';
            echo '<div id="redux-opts-field-errors">' . wp_kses( __('<strong><span></span> error(s) were found!</strong>', 'pantograph'), $allowed_html_array ) . '</div>';
            echo '<div id="redux-opts-field-warnings">' . wp_kses( __('<strong><span></span> warning(s) were found!</strong>', 'pantograph'), $allowed_html_array ).'</div>';
            ...
            foreach($this->sections as $k => $section) {
                ...
                echo '<li id="' . esc_attr( $k ) . '_section_group_li" class="redux-opts-group-tab-link-li">';
                echo '<a href="javascript:void(0);" id="' . esc_attr( $k ) . '_section_group_li_a" class="redux-opts-group-tab-link-a" data-rel="' . esc_attr( $k ) . '">' . $icon . '<span>' . esc_attr( $section['title'] ) . '</span></a>';
                echo '</li>';
            }
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-opts-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span>' . esc_html__('Import / Export', 'pantograph') . '</span></a>';
                echo '</li>';
            ...
            if(is_array($this->extra_tabs)) {
				foreach($this->extra_tabs as $k => $tab) {
                    ...
                    echo '<li id="' . esc_attr( $k ) . '_section_group_li" class="redux-opts-group-tab-link-li">';
                    echo '<a href="javascript:void(0);" id="' . esc_attr( $k ) . '_section_group_li_a" class="redux-opts-group-tab-link-a custom-tab" data-rel="' . esc_attr( $k ) . '">' . $icon . '<span>' . esc_attr( $tab['title'] ) . '</span></a>';
                    echo '</li>';
                }
            }
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="redux-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span>' . esc_html__('Dev Mode Info', 'pantograph') . '</span></a>';
                echo '</li>';
            }
            ...
            foreach($this->sections as $k => $section) {
                echo '<div id="' . esc_attr( $k ) . '_section_group' . '" class="redux-opts-group-tab">';
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<h3>' . esc_html__('Import / Export Options', 'pantograph') . '</h3>';
                echo '<h4>' . esc_html__('Import Options', 'pantograph') . '</h4>';
                echo '<p><a href="javascript:void(0);" id="redux-opts-import-code-button" class="button-secondary">' . esc_html__('Import from file', 'pantograph') . '</a> <a href="javascript:void(0);" id="redux-opts-import-link-button" class="button-secondary">' . esc_html__('Import from URL', 'pantograph') . '</a></p>';

                echo '<div id="redux-opts-import-code-wrapper">';

                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters('redux-opts-import-file-description', esc_html__('Input your backup file below and hit Import to restore your sites options from a backup.', 'pantograph')) . '</p>';
                echo '</div>';
                ...
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters('redux-opts-import-link-description', esc_html__('Input the URL to another sites options set and hit Import to load the options from that site.', 'pantograph')) . '</p>';
                echo '</div>';
                ...
                echo '<p id="redux-opts-import-action"><input type="submit" id="redux-opts-import" name="' . esc_attr( $this->args['opt_name'] ) . '[import]" class="button-primary" value="' . esc_html__('Import', 'pantograph') . '"> <span>' . apply_filters('redux-opts-import-warning', esc_html__('WARNING! This will overwrite any existing options, please proceed with caution!', 'pantograph')) . '</span></p>';
                echo '<div id="import_divide"></div>';

                echo '<h4>' . esc_html__('Export Options', 'pantograph') . '</h4>';
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description">' . apply_filters('redux-opts-backup-description', esc_html__('Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'pantograph')).'</p>';
                echo '</div>';

                echo '<p><a href="javascript:void(0);" id="redux-opts-export-code-copy" class="button-secondary">' . esc_html__('Copy', 'pantograph') . '</a> <a href="'.add_query_arg(array('feed' => 'reduxopts-'.esc_attr( $this->args['opt_name'] ), 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" id="redux-opts-export-code-dl" class="button-primary">Download</a> <a href="javascript:void(0);" id="redux-opts-export-link" class="button-secondary">' . esc_html__('Copy Link', 'pantograph') . '</a></p>';
                ...
                echo '</textarea>';
                echo '<input type="text" class="large-text" id="redux-opts-export-link-value" value="' . add_query_arg(array('feed' => 'reduxopts-' . esc_attr( $this->args['opt_name'] ), 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()) . '" />';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<h3>' . esc_html__('Dev Mode Info', 'pantograph') . '</h3>';
                echo '<div class="redux-opts-section-desc">';
            ...
            if(isset($this->args['share_icons'])) {
                ...
                foreach($this->args['share_icons'] as $link) {
                    echo '<a href="' . esc_url( $link['link'] ) . '" title="' . $link['title'] . '" target="_blank"><img src="' . esc_url( $link['img'] ) . '"/></a>';
                }
                echo '</div>';
            }
            ...
            echo '</form>';
            echo (isset($this->args['footer_text'])) ? '<div id="redux-opts-sub-footer">' . esc_attr( $this->args['footer_text'] ) . '</div>' : '';
        ...
        function _errors_js() {
            if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('redux-opts-errors-' . $this->args['opt_name'])) {
                ...
                foreach($section_errors as $sectionkey => $section_error) {
                    echo 'jQuery("#' . esc_attr( $sectionkey ) . '_section_group_li_a").append("<span class=\"redux-opts-menu-error\">' . esc_attr( $section_error ) . '</span>");';
                }

                foreach($errors as $error) {
                    echo 'jQuery("#' . esc_attr( $error['id'] ) . '").addClass("redux-opts-field-error");';
                    echo 'jQuery("#' . esc_attr( $error['id'] ) . '").closest("td").append("<span class=\"redux-opts-th-error\">' . esc_attr( $error['msg'] ) . '</span>");';
                }
                echo '});';
                echo '</script>';
        ...
        function _warnings_js() {
            if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('redux-opts-warnings-' . $this->args['opt_name'])) {
                ...
                foreach($section_warnings as $sectionkey => $section_warning) {
                    echo 'jQuery("#' . esc_attr( $sectionkey ) . '_section_group_li_a").append("<span class=\"redux-opts-menu-warning\">' . esc_attr( $section_warning ) . '</span>");';
                }

                foreach($warnings as $warning) {
                    echo 'jQuery("#' . esc_attr( $warning['id'] ) . '").addClass("redux-opts-field-warning");';
                    echo 'jQuery("#' . esc_attr( $warning['id'] ) . '").closest("td").append("<span class=\"redux-opts-th-warning\">' . esc_attr( $warning['msg'] ) . '</span>");';
                }
                echo '});';
                echo '</script>';
