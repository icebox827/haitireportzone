...
?>	
<div class="pionusblog inner-content<?php if($pionusnews_hide_title==1){ ?> pzero<?php } ?> two-column-general blog-two-columns-standard <?php if( ($global_white_conainter==1)){ echo esc_html('container no-padding-left-right', 'pantograph');} ?>">
	<?php
		...
		<div class="row">
			<div class="col-md-8">
					...
					$readmorebtn = esc_html__('Read More', 'pantograph');
					    ...
					    $postcontent .= '<div class="post oddeven postcontent masonry-item-new">';
						if ( has_post_format( 'video' )) {
							...
							$postcontent .= '<div class="post-excerpt">
									...
									<div class="meta">
										...
										<span class="comment">
											<a class="meta-link" href="'.get_the_permalink().'"><i class="far fa-comment"></i> '.get_comments_number().'</a>
											In
										</span>
										<span class="small-title cat">' . pionusnews_category_styles() . '</span>
									</div>
										<p>'.pionusnews_theme_content(15).'</p>
									<a href="'.get_permalink().'" class="small-title rmore">'.$readmorebtn.'</a>
								</div>';		
						}else{
							if($get_image){
							$postcontent .= '
