...
*************************/
class pionusnews_subscription_widget extends WP_Widget {

  function __construct() {
    parent::__construct(
      'Pionusnews_subscription', // Base ID
  ...
  $marginstyle ='style="';
  if($margintop != ''){
  $marginstyle .='margin-top:'.esc_attr($margintop).';';
  } if($marginbottom != ''){
  $marginstyle .='margin-bottom:'.esc_attr($marginbottom).';';
  }
  $marginstyle .='"';
  }

  $before_widget = str_replace('class="', ''. $marginstyle . ' class="', $before_widget);
  echo $before_widget; 
  ?>
  <div class="side-widget subscription<?php echo esc_attr($current_time); ?>">
  <?php if($title){ ?>
  <h4><span class="sh-text"><?php echo esc_attr($title); ?></span></h4>
  <?php } ?>
  <div class="side-newsletter text-center">
		<p><?php echo esc_attr( pionusnews_get_option('subscribe_form_heading') ); ?></p>
  <?php if(pionusnews_get_option('ft_aweber_listid') != '') { ?>
  <form method="post" action="https://www.aweber.com/scripts/addlead.pl">
		<input type="hidden" name="redirect" value="<?php echo esc_url( pionusnews_get_option('aweber_redirectpage') ); ?>" />
		<input type="hidden" name="meta_redirect_onlist" value="<?php echo esc_url( pionusnews_get_option('aweber_redirectpage_old') ); ?>" />
  ...
  <form id="sidebar_signup" action="" method="post">
		<input name="email" class="e-mail" id="email_address" placeholder="<?php esc_html_e('Your email address', 'pantograph');?>" type="email" required />
		<button type="submit"><?php esc_html_e('Sign up', 'pantograph');?></button>
	  <div id="sidebar_response" class="result"></div>
	</form>
 ...
 <form method="post" action="#">
  ...
  <?php } ?>
		<span><?php echo esc_attr( pionusnews_get_option('subscribe_form_subheading') ); ?></span>
	</div>
  </div>
...
// register pionusnews_subscription_widget widget
if ( ! function_exists( 'pionusnews_register_subscription_Widget' ) ) {
function pionusnews_register_subscription_Widget() {
  register_widget( 'pionusnews_subscription_widget' );  // Class Name
}
add_action( 'widgets_init', 'pionusnews_register_subscription_Widget' );
}

/*************************
Start Featured Post widget
*************************/
class pionusnews_featured_posts_widget extends WP_Widget {

  function __construct() {
    parent::__construct(
      'Pionusnews_featured_posts', // Base ID
  ...
  echo $before_widget; 
  ?>
  <div class="side-widget featuredposts<?php echo esc_attr($current_time); ?>">
  <?php if($title){ ?>
  <h4><span class="sh-text"><?php echo esc_attr($title); ?></span></h4>
  <?php } ?>
  ...
  <?php
	...
	/*
	if ( ! empty($the_query) && is_array($the_query) ){echo "Ok array";}else{echo "No array";}
	if ($the_query->found_posts){echo "Yes";}else{echo "No";}
	*/
	if($the_query->found_posts){$totalpostcount = $the_query->found_posts;}
	...
	if($showloadmore >= 1){
	?>
	<div class="featured_posts_loadmore<?php echo esc_attr($current_time); ?> loadmorebtn"><?php esc_html_e('Load More', 'pantograph'); ?> <i class="fas fa-redo" aria-hidden="true"></i></div>
	<script type="text/javascript">
	var ajaxurl = "<?php echo admin_url( 'admin-ajax.php' ); ?>";
	//var page = 2; //Removed by Mazhar
	//Ammendment Start by Mazhar
	//var cat = $loader.data('category');
	var ppp = 2;
	var preoffset = jQuery('.featured-posts<?php echo esc_attr($current_time); ?>').find('article').length;
	var offset = preoffset + 1;
	...
	$('body').on('click', '.featured_posts_loadmore<?php echo esc_attr($current_time); ?>', function() {
		...
		$(".featured_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").addClass("fa-spin");

		$.post(ajaxurl, data, function(response) {
			if(response != '') {
			$('.featured-posts<?php echo esc_attr($current_time); ?>').append(response);
			ppp++; //Edited by Mazhar
			$(".featured_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").removeClass("fa-spin");
			} else {
...
// register pionusnews_featured_posts_widget widget
if ( ! function_exists( 'pionusnews_register_featured_posts_Widget' ) ) {
function pionusnews_register_featured_posts_Widget() {
  register_widget( 'pionusnews_featured_posts_widget' );  // Class Name
}
add_action( 'widgets_init', 'pionusnews_register_featured_posts_Widget' );
}

/*************************
Start Popular Post widget
*************************/
class pionusnews_popular_posts_widget extends WP_Widget {

  function __construct() {
    parent::__construct(
      'Pionusnews_popular_posts', // Base ID
  ...
  public function widget( $args, $instance ) {
	  extract($args);
      $title = isset($instance['title']) ? $instance['title'] : '';
	  $certain_cat = isset($instance['certain_cat']) ? $instance['certain_cat'] : '';
	  $post_limit = isset($instance['post_limit']) ? $instance['post_limit'] : '';
	  $wordcount = isset($instance['wordcount']) ? $instance['wordcount'] : '';
	  $load_more = isset($instance['load_more']) ? $instance['load_more'] : '';
  ...
  echo $before_widget; 
  ?>
  <div class="side-widget popularposts<?php echo esc_attr($current_time); ?>">
  <?php if($title){ ?>
  <h4><span class="sh-text"><?php echo esc_attr($title); ?></span></h4>
  <?php } ?>
	<div class="popular-posts<?php echo esc_attr($current_time); ?> mini-posts">
  <?php
	$the_query = new WP_Query( array('cat' => $certain_cat, 'posts_per_page' => $post_limit, 'ignore_sticky_posts' => 1, 'paged' => 1, 'orderby' => 'comment_count'));
	$recent_post_num = 1;
	...
	if ( has_post_format( 'video' ) ) : 
						$videoicon = '<div class="play_thumb_48" style="background:none;"><a href="'.get_permalink().'"><img src="'.get_template_directory_uri().'/template-parts/pionus/img/video.png" alt="" style="width:36px;height:36px;" /></a></div>';
					else:
						$videoicon = '';
					endif;
	?>
		...
		<div class="row">
			...
			<div class="col-md-8 col-sm-8">
				<div class="post-excerpt no-padding">
					...
					<h5><a href="<?php the_permalink(); ?>"><?php if ($wordcount != '') { echo wp_html_excerpt (get_the_title(), $wordcount); } else { echo get_the_title(); } ?></a></h5>
					<?php if(get_comments_number()>0){ ?>
					<div class="meta">
						<span class="comment">
						<a href="<?php the_permalink(); ?>"><i class="far fa-comment"></i> <?php echo get_comments_number(); ?></a>
						</span>
					</div>
					<?php } ?>
				</div>
	...
	<?php if($instance['load_more']==1){ ?>
	<div class="popular_posts_loadmore<?php echo esc_attr($current_time); ?> loadmorebtn"><?php esc_html_e('Load More', 'pantograph'); ?> <i class="fas fa-redo" aria-hidden="true"></i></div>
	<script type="text/javascript">
	var ajaxurl = "<?php echo admin_url( 'admin-ajax.php' ); ?>";
	//var page = 2;
	var ppp = 2;
	var preoffset = jQuery('.popular-posts<?php echo esc_attr($current_time); ?>').find('article').length;
	var offset = preoffset + 1;
	jQuery(function($) {
	$('body').on('click', '.popular_posts_loadmore<?php echo esc_attr($current_time); ?>', function() {
		...
		$(".popular_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").addClass("fa-spin");

		$.post(ajaxurl, data, function(response) {
			if(response != '') {
			$('.popular-posts<?php echo esc_attr($current_time); ?>').append(response);
			ppp++;
			$(".popular_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").removeClass("fa-spin");
			} else {
  ...
  function update( $new_instance, $old_instance ){
    ...
    $instance = $old_instance;
    $instance['title']                = strip_tags( $new_instance['title'] );
    $instance['certain_cat']            = strip_tags( $new_instance['certain_cat'] );
	$instance['post_limit']            = strip_tags( $new_instance['post_limit'] );
  ...
  ?>
    ...
    <p>
	<select id="<?php echo esc_attr($this->get_field_id( 'certain_cat' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'certain_cat' )); ?>" class="widefat">
		  <option value=""><?php esc_html_e('Post Category', 'pantograph'); ?></option>
		  <?php 
		  $post_categories=get_categories();
		  foreach($post_categories as $category) { 
		  ?>
		  <option value="<?php echo $category->cat_ID; ?>" <?php if($instance['certain_cat']==$category->cat_ID){ echo 'selected'; } ?>><?php echo $category->name; ?></option>
		  <?php } ?>
	</select>
	</p>
	...
	<p>
      <label for="<?php echo esc_attr($this->get_field_id( 'wordcount' )); ?>"><?php esc_html_e('Title Character Limit', 'pantograph'); ?></label>
      <input type="text" id="<?php echo esc_attr($this->get_field_id( 'wordcount' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'wordcount' )); ?>" class="widefat" value="<?php echo esc_attr($instance['wordcount']); ?>">
	  <?php esc_html_e('Input a number here. For example if you input 20 here, the title will show up to 20 character at front-end. Keep it empty if you do not want to limit the character of Post Title.', 'pantograph'); ?>
    </p>
...
// register pionusnews_popular_posts_widget widget
if ( ! function_exists( 'pionusnews_register_popular_posts_Widget' ) ) {
function pionusnews_register_popular_posts_Widget() {
  register_widget( 'pionusnews_popular_posts_widget' );  // Class Name
}
add_action( 'widgets_init', 'pionusnews_register_popular_posts_Widget' );
}

/*************************
Start Tab Post widget
*************************/
class pionusnews_tab_posts_widget extends WP_Widget {

  function __construct() {
    parent::__construct(
      'Pionusnews_tab_posts', // Base ID
  ...
  public function widget( $args, $instance ) {
	  extract($args);
	  $pionusnews_tab_widget_title = isset($instance['pionusnews_tab_widget_title']) ? $instance['pionusnews_tab_widget_title'] : '';
	  $pionusnews_popular_title = isset($instance['pionusnews_popular_title']) ? $instance['pionusnews_popular_title'] : '';
	  $pionusnews_commented_title = isset($instance['pionusnews_commented_title']) ? $instance['pionusnews_commented_title'] : '';
	  $commentedpost_limit = isset($instance['commentedpost_limit']) ? $instance['commentedpost_limit'] : '';
	  $pionusnews_viewed_title = isset($instance['pionusnews_viewed_title']) ? $instance['pionusnews_viewed_title'] : '';
  ...
  echo $before_widget; 
  ?>
  <div class="side-widget tabposts<?php echo esc_attr($current_time); ?>">
		<?php if($pionusnews_tab_widget_title){ ?>
		<h4><span class="sh-text"><?php echo esc_attr($pionusnews_tab_widget_title); ?></span></h4>
		<?php } ?>
		<div role="tabpanel" class="thetabpanel">
			<!-- Nav tabs -->
			<?php if(($pionusnews_popular_title !='') || ($pionusnews_commented_title !='') || ($pionusnews_viewed_title !='') ) { ?>
			<ul class="nav nav-tabs nav-justified" role="tablist">
				<li role="presentation" class="active">
					<a href="#popular<?php echo esc_attr($current_time); ?>" aria-controls="popular<?php echo esc_attr($current_time); ?>" role="tab" data-toggle="tab"><?php echo esc_attr($pionusnews_popular_title); ?></a>
				</li>
				<li role="presentation">
					<a href="#commented<?php echo esc_attr($current_time); ?>" aria-controls="commented<?php echo esc_attr($current_time); ?>" role="tab" data-toggle="tab"><?php echo esc_attr($pionusnews_commented_title); ?></a>
				</li>
				<li role="presentation">
					<a href="#viewed<?php echo esc_attr($current_time); ?>" aria-controls="viewed<?php echo esc_attr($current_time); ?>" role="tab" data-toggle="tab"><?php echo esc_attr($pionusnews_viewed_title); ?></a>
				</li>
			</ul>
			<?php } ?>
			...
			<div class="tab-content">
				...
				<?php
					...
					if ( has_post_format( 'video' ) ) : 
						$videoicon = '<div class="play_thumb_48" style="background:none;"><a href="'.get_permalink().'"><img src="'.get_template_directory_uri().'/template-parts/pionus/img/video.png" alt="" style="width:48px;height:48px;" /></a></div>';
					else:
						$videoicon = '';
					endif;
					...
					<?php if($instance['load_more']==1){ ?>
					<div class="popular_tab_posts_loadmore<?php echo esc_attr($current_time); ?> loadmorebtn"><?php esc_html_e('Load More', 'pantograph'); ?> <i class="fas fa-redo" aria-hidden="true"></i></div>
					<script type="text/javascript">
						var ajaxurl = "<?php echo admin_url( 'admin-ajax.php' ); ?>";
						var page = 2;
						jQuery(function($) {
						$('body').on('click', '.popular_tab_posts_loadmore<?php echo esc_attr($current_time); ?>', function() {
							...
							$(".popular_tab_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").addClass("fa-spin");

							$.post(ajaxurl, data, function(response) {
								if(response != '') {
								$('.popular-tab-posts<?php echo esc_attr($current_time); ?>').append(response);
								page++;
								$(".popular_tab_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").removeClass("fa-spin");
								} else {
				...
				<?php
					...
					<?php if($instance['load_more']==1){ ?>
					<div class="comment_tab_posts_loadmore<?php echo esc_attr($current_time); ?> loadmorebtn"><?php esc_html_e('Load More', 'pantograph'); ?> <i class="fas fa-redo" aria-hidden="true"></i></div>
					<script type="text/javascript">
						var ajaxurl = "<?php echo admin_url( 'admin-ajax.php' ); ?>";
						var page = 2;
						jQuery(function($) {
						$('body').on('click', '.comment_tab_posts_loadmore<?php echo esc_attr($current_time); ?>', function() {
							...
							$(".comment_tab_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").addClass("fa-spin");

							$.post(ajaxurl, data, function(response) {
								if(response != '') {
								$('.comment-tab-posts<?php echo esc_attr($current_time); ?>').append(response);
								page++;
								$(".comment_tab_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").removeClass("fa-spin");
								} else {
				...
				<div role="tabpanel" class="tab-pane fade in" id="viewed<?php echo esc_attr($current_time); ?>">
					...
					<?php if($instance['load_more']==1){ ?>
					<div class="viewed_tab_posts_loadmore<?php echo esc_attr($current_time); ?> loadmorebtn"><?php esc_html_e('Load More', 'pantograph'); ?> <i class="fas fa-redo" aria-hidden="true"></i></div>
					<script type="text/javascript">
						var ajaxurl = "<?php echo admin_url( 'admin-ajax.php' ); ?>";
						var page = 2;
						jQuery(function($) {
						$('body').on('click', '.viewed_tab_posts_loadmore<?php echo esc_attr($current_time); ?>', function() {
							...
							$(".viewed_tab_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").addClass("fa-spin");

							$.post(ajaxurl, data, function(response) {
								if(response != '') {
								$('.viewed-tab-posts<?php echo esc_attr($current_time); ?>').append(response);
								page++;
								$(".viewed_tab_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").removeClass("fa-spin");
								} else {
...
<?php echo $after_widget; ?>
  ...
  function update( $new_instance, $old_instance ){
    ...
    $instance = $old_instance;
    $instance['pionusnews_tab_widget_title']  = strip_tags( $new_instance['pionusnews_tab_widget_title'] );
    $instance['pionusnews_popular_title']  = strip_tags( $new_instance['pionusnews_popular_title'] );
    $instance['popular_post_limit']            = strip_tags( $new_instance['popular_post_limit'] );
	$instance['pionusnews_commented_title']  = strip_tags( $new_instance['pionusnews_commented_title'] );
    $instance['commentedpost_limit']            = strip_tags( $new_instance['commentedpost_limit'] );
	$instance['pionusnews_viewed_title']                = strip_tags( $new_instance['pionusnews_viewed_title'] );
  ...
  function form($instance){
    $defaults = array( 
      'pionusnews_tab_widget_title'               => '',
      'pionusnews_popular_title'               => '',
      'popular_post_limit'           => 3,
	  'pionusnews_commented_title'               => '',
      'commentedpost_limit'           => 3,
	  'pionusnews_viewed_title'               => '',
      'viewd_post_limit'           => 3,
      'load_more'           => 0,
  ...
  ?>
    <p>
      <label for="<?php echo esc_attr($this->get_field_id( 'pionusnews_tab_widget_title ' )); ?>"><?php esc_html_e('Widget Heading', 'pantograph'); ?></label>
      <input type="text" id="<?php echo esc_attr($this->get_field_id( 'pionusnews_tab_widget_title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'pionusnews_tab_widget_title' )); ?>" class="widefat" value="<?php echo esc_attr($instance['pionusnews_tab_widget_title']); ?>">
    </p>
	
	<p>
      <label for="<?php echo esc_attr($this->get_field_id( 'pionusnews_popular_title ' )); ?>"><?php esc_html_e('Popular Tab Heading', 'pantograph'); ?></label>
      <input type="text" id="<?php echo esc_attr($this->get_field_id( 'pionusnews_popular_title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'pionusnews_popular_title' )); ?>" class="widefat" value="<?php echo esc_attr($instance['pionusnews_popular_title']); ?>">
    </p>
	...
	<p>
      <label for="<?php echo esc_attr($this->get_field_id( 'pionusnews_commented_title' )); ?>"><?php esc_html_e('Commented Tab Heading', 'pantograph'); ?></label>
      <input type="text" id="<?php echo esc_attr($this->get_field_id( 'pionusnews_commented_title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'pionusnews_commented_title' )); ?>" class="widefat" value="<?php echo esc_attr($instance['pionusnews_commented_title']); ?>">
    </p>
	...
	<p>
      <label for="<?php echo esc_attr($this->get_field_id( 'pionusnews_viewed_title' )); ?>"><?php esc_html_e('Viewed Tab Heading', 'pantograph'); ?></label>
      <input type="text" id="<?php echo esc_attr($this->get_field_id( 'pionusnews_viewed_title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'pionusnews_viewed_title' )); ?>" class="widefat" value="<?php echo esc_attr($instance['pionusnews_viewed_title']); ?>">
    </p>
...
// register pionusnews_tab_posts_widget widget
if ( ! function_exists( 'pionusnews_register_tab_posts_Widget' ) ) {
function pionusnews_register_tab_posts_Widget() {
  register_widget( 'pionusnews_tab_posts_widget' );  // Class Name
}
add_action( 'widgets_init', 'pionusnews_register_tab_posts_Widget' );
}

/*************************
Start Video Post widget
*************************/
class pionusnews_video_posts_widget extends WP_Widget {

  function __construct() {
    parent::__construct(
      'Pionusnews_video_posts', // Base ID
  ...
  echo $before_widget; 
  ?>
  <div class="side-widget videoposts<?php echo esc_attr($current_time); ?>">
  <?php if($title){ ?>
  <h4><span class="sh-text"><?php echo esc_attr($title); ?></span></h4>
  <?php } ?>
  ...
  <?php
	...
	*/
	...
	if($showloadmore >= 1){ 
	?>
	<div class="video_posts_loadmore<?php echo esc_attr($current_time); ?> loadmorebtn"><?php esc_html_e('Load More', 'pantograph'); ?> <i class="fas fa-redo" aria-hidden="true"></i></div>
	<script type="text/javascript">
		var ajaxurl = "<?php echo admin_url( 'admin-ajax.php' ); ?>";
		//var page = 2;
		var ppp = 2;
		var preoffset = jQuery('.video-posts<?php echo esc_attr($current_time); ?>').find('article').length;
		var offset = preoffset + 1;
		jQuery(function($) {
		$('body').on('click', '.video_posts_loadmore<?php echo esc_attr($current_time); ?>', function() {
			...
			$(".video_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").addClass("fa-spin");

			$.post(ajaxurl, data, function(response) {
				if(response != '') {
				$('.video-posts<?php echo esc_attr($current_time); ?>').append(response);
				ppp++;
				$(".video_posts_loadmore<?php echo esc_attr($current_time); ?>.loadmorebtn i").removeClass("fa-spin");
				} else {
...
// register pionusnews_video_posts_widget widget
if ( ! function_exists( 'pionusnews_register_video_posts_Widget' ) ) {
function pionusnews_register_video_posts_Widget() {
  register_widget( 'pionusnews_video_posts_widget' );  // Class Name
}
add_action( 'widgets_init', 'pionusnews_register_video_posts_Widget' );
}

/*************************
Start Include Category widget
*************************/
class pionusnews_include_category_widget extends WP_Widget {

  function __construct() {
    parent::__construct(
      'pionusnews_include_category', // Base ID
...
<?php
	...
	foreach($Parent_categories as $category) {
		...
		echo "<li>There is no selected category</li>";	
		}
?>
        </ul>
		<style>
...
// register pionusnews_include_category_widget widget
if ( ! function_exists( 'pionusnews_register_include_category_Widget' ) ) {
function pionusnews_register_include_category_Widget() {
  register_widget( 'pionusnews_include_category_widget' );  // Class Name
}
add_action( 'widgets_init', 'pionusnews_register_include_category_Widget' );
}