...
$MailChimp = new MailChimp(pionusnews_get_option('mailchimp_apikey'));

$mailchimp_result = $MailChimp->call('lists/subscribe', array(
													'id'                => pionusnews_get_option('mailchimp_listid'),
...
if (isset($mailchimp_result['leid'])) {
	echo '<p><b>'.esc_html__('Thank You for Subscribing!', 'pantograph').'</b></p>';
	echo '<p>'.esc_html__('You must confirm your email address, Please check email and follow the instructions.', 'pantograph').'</p>';
} else {
	echo '<b>'.esc_html__('Please, Input a Valid Email', 'pantograph').'</b>';
}
