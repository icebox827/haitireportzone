...
class pionusnews_menu_logo {
	...
	public function __construct() {
		// Register init
		add_action( 'init', array( $this, 'pionusnews_menu_logo_init' ) );
	 ...
	 * @uses pionusnews_menu_logo_admin() To call admin functions.
	 * @uses add_action() To hook function.
	 ...
	 */
	public function pionusnews_menu_logo_init() {
		...
		if ( is_admin() ) {
			...
			new pionusnews_menu_logo_admin();
		}

		// Register AJAX handler
		add_action( 'wp_ajax_nmi_added_thumbnail', array( $this, 'ajax_added_thumbnail' ) );
		add_action( 'wp_ajax_nopriv_nmi_added_thumbnail', array( $this, 'ajax_added_thumbnail' ) );

		// Register menu item content filter if needed
		if ( apply_filters( 'nmi_filter_menu_item_content', true ) ) {
			add_filter( 'nav_menu_css_class',       array( $this, 'pionusnews_menu_logo_register_menu_item_filter'   ), 15, 3 );
			add_filter( 'walker_nav_menu_start_el', array( $this, 'pionusnews_menu_logo_depionusnews_menu_logo_register_menu_item_filter' ), 15, 2 );
		}
	 ...
	 */
	public function pionusnews_menu_logo_load_textdomain() {
		/* If translation isn't loaded, load it */
		if ( ! is_textdomain_loaded( 'pionusnews_menu_logo' ) )
			load_plugin_textdomain( 'pionusnews_menu_logo', false, dirname( $this->plugin_basename ) . '/languages' );
	}
	 ...
	 */
	public function pionusnews_menu_logo_register_menu_item_filter( $item_classes, $item, $args = array() ) {
	 ...
	 */
	public function pionusnews_menu_logo_depionusnews_menu_logo_register_menu_item_filter( $item_output, $item ) {
 ...
 * @since 1.0
 *
 * @uses pionusnews_menu_logo To initialize plugin.
 */
function pionusnews_menu_logo_instantiate() {
	new pionusnews_menu_logo();
}
add_action( 'plugins_loaded', 'pionusnews_menu_logo_instantiate', 15 );
