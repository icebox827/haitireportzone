...
class Importer {
	...
	/**
	 * The instance of the OCDC\Logger class.
	 *
	 * @var object
	 */
	public $logger;
	...
	public function __construct( $importer_options = array(), $logger = null ) {
		...
		// Get the OCDC (main plugin class) instance.
	...
	public function import_content( $import_file_path ) {
		...
		set_time_limit( apply_filters( 'pt-ocdc/set_time_limit_for_demo_data_import', 300 ) );
		...
		if ( ! apply_filters( 'pt-ocdc/regenerate_thumbnails_in_content_import', true ) ) {
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-ocdc/time_for_one_ajax_call', 25 ) ) {
			$response = array(
				'status'  => 'newAJAX',
			...
			if ( ! empty( $message ) ) {
				$this->ocdc->append_to_frontend_error_messages( $message );
			}

			// Add message to log file.
			$log_added = Helpers::append_to_file(
				__( 'New AJAX call!' , 'pantograph' ) . PHP_EOL . $message,
				$this->ocdc->get_log_file_path(),
				''
			);
	...
	private function set_current_importer_data() {
		$data = array_merge( $this->ocdc->get_current_importer_data(), $this->get_importer_data() );

		Helpers::set_ocdc_import_data_transient( $data );
	}
}
