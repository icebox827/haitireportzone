...
class Redux_Options_upload {
    ...
    */
    function render() {
        ...
        echo '<img class="redux-opts-screenshot" id="redux-opts-screenshot-' . esc_attr( $this->field['id'] ) . '" src="' . esc_url( $this->value ) . '" />';
        if($this->value == '') {$remove = ' style="display:none;"'; $upload = ''; } else {$remove = ''; $upload = ' style="display:none;"'; }
        echo ' <a data-update="Select File" data-choose="Choose a File" href="javascript:void(0);"class="redux-opts-upload button-secondary"' . $upload . ' rel-id="' . esc_attr( $this->field['id'] ) . '">' . esc_html__('Upload', 'pantograph') . '</a>';
        echo ' <a href="javascript:void(0);" class="redux-opts-upload-remove"' . $remove . ' rel-id="' . esc_attr( $this->field['id'] ) . '">' . esc_html__('Remove Upload', 'pantograph') . '</a>';
