...
class OneClickDemoImport {
	...
	/**
	 * The instance of the OCDC\Importer class.
	 *
	 * @var object
	 */
	public $importer;
	...
	protected function __construct() {
		...
		add_action( 'wp_ajax_ocdc_import_demo_data', array( $this, 'import_demo_data_ajax_callback' ) );
		add_action( 'wp_ajax_nopriv_ocdc_import_demo_data', array( $this, 'import_demo_data_ajax_callback' ) );
		add_action( 'wp_ajax_ocdc_import_customizer_data', array( $this, 'import_customizer_data_ajax_callback' ) );
		add_action( 'wp_ajax_nopriv_ocdc_import_customizer_data', array( $this, 'import_customizer_data_ajax_callback' ) );
		add_action( 'wp_ajax_ocdc_after_import_data', array( $this, 'after_all_import_data_ajax_callback' ) );
		add_action( 'wp_ajax_nopriv_ocdc_after_import_data', array( $this, 'after_all_import_data_ajax_callback' ) );
	...
	public function create_plugin_page() {
		$plugin_page_setup = apply_filters( 'pt-ocdc/plugin_page_setup', array(
				'parent_slug' => 'themes.php',
				'page_title'  => esc_html__( 'One Click Demo Import' , 'pantograph' ),
				'menu_title'  => esc_html__( 'Import Demo' , 'pantograph' ),
				'capability'  => 'import',
				'menu_slug'   => 'pt-one-click-demo-content',
			)
		);
		...
		$this->plugin_page = add_submenu_page(
			...
			apply_filters( 'pt-ocdc/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) )
		);
	}
	...
	public function display_plugin_page() {
		require_once PT_OCDC_PATH . 'one-click-demo-content/views/plugin-page.php';
	}
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook ) {
			...
			wp_enqueue_script( 'ocdc-main-js', PT_OCDC_URL . 'one-click-demo-content/assets/js/main.js' , array( 'jquery', 'jquery-ui-dialog' ), PT_OCDC_VERSION );

			// Get theme data.
			$theme = wp_get_theme();

			wp_localize_script( 'ocdc-main-js', 'ocdc',
				array(
					'ajax_url'         => admin_url( 'admin-ajax.php' ),
					'ajax_nonce'       => wp_create_nonce( 'ocdc-ajax-verification' ),
					'import_files'     => $this->import_files,
					'wp_customize_on'  => apply_filters( 'pt-ocdc/enable_wp_customize_save_hooks', false ),
					'import_popup'     => apply_filters( 'pt-ocdc/enable_grid_layout_import_popup_confirmation', true ),
					'theme_screenshot' => $theme->get_screenshot(),
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'pantograph' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'pantograph' ),
						'dialog_no'             => esc_html__( 'Cancel', 'pantograph' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'pantograph' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'pantograph' ),
					),
					'dialog_options' => apply_filters( 'pt-ocdc/confirmation_dialog_options', array() )
				)
			);

			wp_enqueue_style( 'ocdc-main-css', PT_OCDC_URL . 'one-click-demo-content/assets/css/main.css', array() , PT_OCDC_VERSION );
		}
	}
	...
	public function import_demo_data_ajax_callback() {
		...
		ini_set( 'memory_limit', apply_filters( 'pt-ocdc/import_memory_limit', '350M' ) );
		...
		if ( ! $use_existing_importer_data ) {
			 ...
			 * Manually uploaded import files or predefined import files via filter: pt-ocdc/import_files
			 */
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'pantograph' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdc/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'pantograph' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'pantograph' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'pantograph' ) );
			}
		}

		// Save the initial import data as a transient, so other import parts (in new AJAX calls) can use that data.
		Helpers::set_ocdc_import_data_transient( $this->get_current_importer_data() );

		if ( ! $this->before_import_executed ) {
			$this->before_import_executed = true;

			/**
			 * 2). Execute the actions hooked to the 'pt-ocdc/before_content_import_execution' action:
			 *
			 * Default actions:
			 * 1 - Before content import WP action (with priority 10).
			 */
			do_action( 'pt-ocdc/before_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
		...
		/**
		 * 4). Execute the actions hooked to the 'pt-ocdc/after_content_import_execution' action:
		 *
		 * Default actions:
		 ...
		 */
		do_action( 'pt-ocdc/after_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		...
		if ( false !== has_action( 'pt-ocdc/after_all_import_execution' ) ) {
	...
	public function import_customizer_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'pt-ocdc/customizer_import_execution', $this->selected_import_files );
		}
	...
	public function after_all_import_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'pt-ocdc/after_all_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
	...
	private function final_response() {
		...
		delete_transient( 'ocdc_importer_data' );

		// Display final messages (success or error messages).
		if ( empty( $this->frontend_error_messages ) ) {
			$response['message'] = '';

			if ( ! apply_filters( 'pt-ocdc/disable_pt_branding', false ) ) {
				$twitter_status = esc_html__( 'Just used One Click Demo Import plugin and it was awesome! Thanks @FluentThemes! #OCDC https://www.fluentthemes.com/', 'pantograph' );

				$response['message'] .= sprintf(
					__( '%1$s%6$sWasn\'t this a great One Click Demo Import experience?%7$s Created and maintained by %3$sFluentThemes%4$s. %2$s%5$sClick to Tweet!%4$s%8$s', 'pantograph' ),
					'<div class="notice  notice-info"><p>',
					'<br>',
					'<strong><a href="https://www.fluentthemes.com/" target="_blank">',
					'</a></strong>',
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sOne Click Demo Import%4$s plugin, because it has done its job.%5$s', 'pantograph' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'pantograph' ),
	...
	private function use_existing_importer_data() {
		if ( $data = get_transient( 'ocdc_importer_data' ) ) {
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'pt-ocdc', false, plugin_basename( dirname( __FILE__ ) ) . '/languages' );
	}
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = Helpers::validate_import_file_info( apply_filters( 'pt-ocdc/import_files', array() ) );

		/**
		 * Register all default actions (before content import, widget, customizer import and other actions)
		 * to the 'before_content_import_execution' and the 'pt-ocdc/after_content_import_execution' action hook.
		 */
		$import_actions = new ImportActions();
		$import_actions->register_hooks();

		// Importer options array.
		$importer_options = apply_filters( 'pt-ocdc/importer_options', array(
			'fetch_attachments' => true,
		) );

		// Logger options for the logger used in the importer.
		$logger_options = apply_filters( 'pt-ocdc/logger_options', array(
