 ...
 * @since 1.0
 *
 * @uses pantograph_menu_logo
 */
class pantograph_menu_logo_admin extends pantograph_menu_logo {
	/**
	 ...
	 *
	 * @global $wp_version.
	 * @uses pantograph_menu_logo::pantograph_menu_logo_load_textdomain() To load translations.
	...
	public function enqueue_scripts() {
		...
		// Load translations
		$this->pantograph_menu_logo_load_textdomain();
		...
		wp_localize_script( 'nmi-scripts', 'nmi_vars', array(
				'alert' => __( 'You need to set an image as a featured image to be able to use it as an menu item image', 'pantograph_menu_logo' )
			)
		);
		add_thickbox();
	...
	public function _wp_post_thumbnail_html( $content, $post_id ) {
		...
		if ( has_post_thumbnail( $post->ID ) )
			...
			$link = __( 'Upload menu item image', 'pantograph_menu_logo' );
		...
		if ( isset( $_REQUEST['thumb_was'] ) && -1 == $_REQUEST['thumb_was'] ) {
			$link_text = __( 'Change menu item image', 'pantograph_menu_logo' );
			$ajax_nonce = wp_create_nonce( 'set_post_thumbnail-' . $post->ID );
			$remove_link = ' | <a href="#" data-id="' . $post->ID . '" class="nmi_remove" onclick="NMIRemoveThumbnail(\'' . $ajax_nonce . '\',' . $post->ID . ');return false;">' . esc_html__( 'Remove menu item image', 'pantograph_menu_logo' ) . '</a>';
	...
	function attachment_fields_to_edit( $form_fields, $post ) {
		...
		$form_fields['nmihover'] = array(
			'label'        => __( 'Used on hover?', 'pantograph_menu_logo' ),
			'input'        => 'html',
			...
			'value'        => $is_hover,
			'helps'        => __( 'Should this image be used on hover', 'pantograph_menu_logo' ),
			'show_in_edit' => false
		);
		...
		$form_fields['nmiactive'] = array(
			'label'        => __( 'Used when active?', 'pantograph_menu_logo' ),
			'input'        => 'html',
			...
			'value'        => $is_active,
			'helps'        => __( 'Should this image be used when menu item is active', 'pantograph_menu_logo' ),
			'show_in_edit' => false
		);
